#
# Thu Jul 14 09:07:38 2022, extract from Ulises by Gonzalo Simarro and Daniel Calvete
#
import copy
import cv2
import itertools
...
def CloudOfPoints2InnerPositionsForPolyline(xs, ys, polyline, options={}): # 202207021000 (last read 2022-07-02, checked graphically with auxiliar code)
    ...
    possIn0 = [item for item in range(len(xsM)) if np.min(np.sqrt((xsM[item]-xsP)**2+(ysM[item]-ysP)**2)) < eps]
    xsC, ysC, possMR0 = np.concatenate((xsP, xsM)), np.concatenate((ysP, ysM)), [item for item in range(len(xsM)) if item not in possIn0]
    while True:
        xO, yO = np.max(xsC) + (0.5 + np.random.random()) * np.std(xsC), np.max(ysC) + (0.5 + np.random.random()) * np.std(ysC)
        ...
        if np.min(np.abs(anglesP - anglesMR0)) > 1.e+1 * options['epsilon']:
            break
...
def UUaVUa2XYZ(mainSet, planes, uUas, vUas, options={}): # 202109141800 (last read 2022-07-12) potentially expensive
    ...
    dens = ClipWithSign(dens, 1.e-8, 1.e+8)
...
def X2EOF(X): # 202002161020 (last read 2022-07-12)
    ...
    U, S, VH = la.svd(XHat, full_matrices=False) # (nx x nq)-ndarray, nq array, nq x nt ndarray
